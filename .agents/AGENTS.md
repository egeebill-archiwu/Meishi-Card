...
* 本專案使用的 Google API Key 屬於新版免費層級：
  * **❌ 不支援模型：** `gemini-1.5-pro`、`gemini-1.5-flash` 舊版系列（為防止辨識落後與定位偏移，全面禁用與下架）。
  * **✅ 支援模型：** `gemini-2.5-pro`、`gemini-2.5-flash`。
* **模型選擇與切換原則：**
  * 使用者可以使用 **`gemini-2.5-flash`** 作為極速版，或選擇 **`gemini-2.5-pro`** 獲得極高精度的長文本與多身份辨識。所有模型調用必須嚴格遵循第 1 條的 `/v1/` 參數規範。
